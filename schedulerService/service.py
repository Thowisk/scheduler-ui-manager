...
def log(task_id, task_info):
    with open(DEFAULT_SERVICE_ROOT + 'logs/LOG', 'a') as file:
        file.write('[' + datetime.datetime.now().__str__() + '] {task_id: ' + str(task_id) + ', return_code: ' + str(task_info['result'].returncode) + ', script: ' + task_info['script'].split('/')[-1] + ' }\n')

def listener(event):
    log(event.job_id.split('_')[-1], event.retval)
